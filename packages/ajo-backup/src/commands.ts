...
Next steps:
  ...
  3. Run: kit backup push -f FOLDER_ID --watch
`)
					server.close()
					process.exit(0)
	...
	cli.command('backup push')
		.describe('Push database to Google Drive (WAL-based)')
		.option('-b, --database', 'Path to SQLite database', DATABASE_PATH)
		...
		.action((opts: { database: string; credentials: string; folder: string; snapshot: string; name: string; watch: boolean; rotate: number; debounce: number }) => {
			...
			const db = new Database(opts.database)
