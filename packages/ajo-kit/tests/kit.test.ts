...
import { Failure, Invalid, ip, normalize, origin } from '../src/constants'
import type { State } from '../src/constants'
import { kit } from '../src/vite'
import { object, parse, string, minLength as min, pipe } from '../src/validate'
import {
	finish,
	header,
	start,
...
import { compile, listen } from '../src/node'
import { migrator } from '../src/migrate'
...
describe('ajo-kit vite plugin', () => {
	...
	test('client-marked modules bypass server-only patterns', async () => {
		...
		await expect(hook.handler.call(context, '/project/node_modules/ajo-auth/src/token.ts', '/project/src/page.tsx')).rejects.toThrow('Server-only module')
		await expect(hook.handler.call(context, '/project/node_modules/ajo-auth/src/ability.client.ts', '/project/src/page.tsx')).resolves.toBeUndefined()
