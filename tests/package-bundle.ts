...
const legacyFloatingMarker = '/ajo-ui/src/floating.ts'
if (existsSync(resolve('packages/ajo-cloves/src/anchor.ts'))) throw new Error('Legacy anchor.ts still exists')
if (existsSync(resolve('packages/ajo-cloves/src/follow.ts'))) throw new Error('Legacy follow.ts still exists')
...
const assertNoLegacyFloating = (result: Result) => {
	if (hasModule(result, legacyFloatingMarker)) throw new Error(`${result.name} retained legacy floating.ts`)
}
...
assertNoLegacyFloating(tooltipDirect)
assertNoLegacyFloating(tooltipRoot)
...
for (const [family, directBundle, rootBundle] of [
	...
	assertNoLegacyFloating(directBundle)
	assertNoLegacyFloating(rootBundle)
